...
def dictionary_add_station(cluster_name):
    ...
    for k in time_intervals_dict.keys():
        data.append([k, time_intervals_dict[k], k.day_name()])
    df_new = pd.DataFrame(data,columns=['Date_Time','Energy', 'Day_of_Week'])
    new_file_name = 'clean_data/Dataset2/' + cluster_name + '.csv'
